...
class HigherLowerBot:
    def __init__(self, token, app_id, symbol, duration, duration_unit, stake, take_profit, stop_loss, martingale_mult, max_martingale_level, barrier_offset):
        self.token = token
        self.app_id = app_id
        self.symbol = symbol
        ...
        self.barrier_offset = abs(float(barrier_offset)) # Force absolute value
    ...
    def setup_logging(self):
        self.logger = logging.getLogger("HigherLowerBot")
        self.logger.setLevel(logging.INFO)
        handler = RotatingFileHandler("higher_lower.log", maxBytes=5*1024*1024, backupCount=2)
    ...
    def calculate_indicators(self):
        ...
        curr = df.iloc[-1]
        return curr, curr['bb_high'], curr['bb_low']

    async def evaluate_strategy(self):
        if self.current_contract_id is not None:
            return

        curr, bb_high, bb_low = self.calculate_indicators()

        if curr is None or pd.isna(bb_high):
            return

        # Place LOWER if high touches/crosses Upper Band
        if curr['high'] >= bb_high:
            print(f"\n{Fore.MAGENTA}[SIGNAL] LOWER TRIGGERED (Touched Upper BB: {bb_high:.2f}){Style.RESET_ALL}")
            # Positive barrier offset
            await self.place_trade("PUT", f"+{self.barrier_offset}")

        # Place HIGHER if low touches/crosses Lower Band
        elif curr['low'] <= bb_low:
            print(f"\n{Fore.MAGENTA}[SIGNAL] HIGHER TRIGGERED (Touched Lower BB: {bb_low:.2f}){Style.RESET_ALL}")
            # Negative barrier offset
            await self.place_trade("CALL", f"-{self.barrier_offset}")

    async def place_trade(self, contract_type, barrier):
        self.current_stake = round(self.current_stake, 2)
        print(f"{Fore.BLUE}[OPEN] Placing {contract_type} | Barrier: {barrier} | Stake: ${self.current_stake}{Style.RESET_ALL}")
